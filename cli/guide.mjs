...
export function guide(connectors) {
  return ['Foundation holds, for each principal, grants and objects, and runs explicitly requested built-in functions.',
    ...
    '   PUT /v1/holdings?kind=grant&name=<name>[&provider=<id>&purpose=<text>]    body: raw bytes, up to 1MB. A given grant:',
    '     the same exact name replaces that value; the answer carries its id. provider names the service it belongs to',
    '     (aws, stripe, github, ...) and purpose what it is for; both are the holder\'s words and may be changed later.',
    ...
    '   GET /v1/holdings?kind=grant    every grant: id, name, method (given / authorized / delegated), provider, purpose, status;',
    '     for a connected one also its service, facts and the variables it yields. &method= &provider= &prefix= narrow it.',
    ...
    '   PATCH /v1/holdings/<id> {"name","provider","purpose"}    new words for the same thing.   DELETE /v1/holdings/<id>   removes a given one.',
