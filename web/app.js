...
function connectToken(provider, request = null) {
  ...
  openDialog(`<h2 id="dialog-title">${esc(serviceName || provider.name)}を接続</h2><p>${setupFields.length ? `${esc(provider.name)}で発行した${esc(setup.label)}と、対象の${setupFields.map(field => esc(field.label)).join('・')}を登録します。` : serviceName ? `パスワードは${esc(serviceName)}の画面で入力します。ここには${esc(setup.label)}だけを登録してください。` : esc(provider.intro)}</p>
    ...
    <div class="token-setup"><h3>1. ${esc(serviceName || 'サービス')}でキーを作成</h3><p>${esc(setup.instructions)}</p>${siteUrl || fields.length ? `<a class="button secondary full" href="${esc(siteUrl || '#')}" target="_blank" rel="noopener noreferrer" ${siteUrl ? '' : 'style="display:none"'}>${esc(setup.link_label || `${serviceName || 'サービス'}の${setup.label}管理ページを開く`)} ↗</a>` : ''}</div>
    <h3 class="token-step">2. キーを登録</h3>${setupInputs}<label for="connection-token">${esc(setup.label)}</label>${setup.multiline ? '<textarea id="connection-token" name="token" rows="6" required minlength="8" maxlength="8192" autocomplete="off" spellcheck="false" autocapitalize="none" aria-describedby="token-storage-note" placeholder="-----BEGIN PRIVATE KEY-----"></textarea>' : '<input id="connection-token" name="token" type="password" required minlength="8" maxlength="4096" autocomplete="off" spellcheck="false" autocapitalize="none" aria-describedby="token-storage-note">'}<p class="permission-note" id="token-storage-note">キーは暗号化して保存し、許可したアクセスキーの持ち主だけに渡します。チャットには貼り付けないでください。</p>${setup.note ? `<p class="permission-note">${esc(setup.note)}</p>` : ''}
