...
const accountLabel = account => account?.label || account?.subject || '';
const adapterOf = account => state.adapters.find(adapter => adapter.id === account.adapter);
// What the owner calls the service: the adapter's service, or for the generic adapter what the runtime declared.
const serviceName = (adapter, details) => adapter.service?.name || details?.service || adapter.label;
const icon = (name) => {
  const paths = {
...
function details(account) {
  if (!account) return '';
  const adapter = adapterOf(account), name = serviceName(adapter, account.details);
  return `<div class="connection-heading"><span class="status ${account.status === 'connected' ? '' : 'warning'}">${account.status === 'connected' ? icon('check') : ''}${statusName(account)}</span>${account.verified === false ? '<span class="status neutral">未検証</span>' : ''}<h3>${esc(account.name)}</h3><p class="account-email">${esc(accountLabel(account))}</p></div>
    <dl class="connection-facts"><div><dt>用途</dt><dd>${esc(account.purpose || '未設定')}</dd></div>${Array.isArray(account.organizations) ? `<div><dt>組織</dt><dd>${account.organizations.length ? account.organizations.map(item => esc(item.name)).join('、') : 'なし'}</dd></div>` : ''}${claimRows(adapter, account.details)}<div><dt>許可範囲</dt><dd>${esc(scopeName(account))}<span class="muted block">${esc(account.permission?.restrictions)}</span></dd></div></dl>${keyFacts(account)}
    <div class="connection-actions">${account.verified === false ? '' : `<button class="button secondary" data-action="check" data-id="${esc(account.id)}" ${account.status !== 'connected' || !adapter.available ? 'disabled' : ''}>接続を確認</button>`}${adapter.can_reconnect ? `<button class="text-button" data-action="reconnect" data-id="${esc(account.id)}" ${account.status === 'disconnecting' || !adapter.available ? 'disabled' : ''}>再接続</button>` : ''}<button class="text-button" data-action="edit-account" data-id="${esc(account.id)}">編集</button></div>
    ${verificationDetails(account.verification)}<details class="connection-reference"><summary>接続情報</summary><dl><dt>接続ID</dt><dd><code>${esc(account.id)}</code></dd></dl>${adapter.service?.api.documentation_url ? `<a href="${esc(adapter.service.api.documentation_url)}" target="_blank" rel="noopener noreferrer">${esc(name)} APIの公式ドキュメント ↗</a>` : ''}</details>
    <div class="connection-footer">${account.credential_type === 'expo_session' ? '' : `<a href="${esc(account.management_url || adapter.service?.management_url)}" target="_blank" rel="noopener noreferrer">${esc(name)}の${adapter.declared ? 'キー管理ページ' : '接続管理'} ↗</a>`}<button class="text-button danger" data-action="remove-account" data-id="${esc(account.id)}">${account.status === 'disconnecting' ? '接続解除を再試行' : '接続を解除'}</button></div>`;
}
// One section per service the owner recognises; credentials registered through any of its adapters sit together.
// Credentials the generic adapter holds sit in a section of their own.
function serviceGroups() {
  const groups = new Map();
  for (const adapter of state.adapters) {
    const key = adapter.service?.id || 'generic';
    if (!groups.has(key)) groups.set(key, { id: key, name: adapter.service?.name || 'その他のキー', icon: adapter.service?.icon || 'key', adapters: [] });
    groups.get(key).adapters.push(adapter);
  }
  return [...groups.values()];
}
function serviceSection(group) {
  const ids = new Set(group.adapters.map(adapter => adapter.id)), accounts = state.accounts.filter(account => ids.has(account.adapter));
  const addable = group.adapters.filter(adapter => !adapter.declared), available = addable.some(adapter => adapter.available);
  const account = accounts.find(item => item.id === selected) || accounts[0];
  return `<section class="resource-section" aria-labelledby="${esc(group.id)}-title"><div class="section-heading"><div class="section-label"><span class="service-icon">${icon(group.icon)}</span><div><h2 id="${esc(group.id)}-title">${esc(group.name)}</h2><p>${accounts.length ? `${accounts.length}件の接続` : '未接続'}</p></div></div>${addable.length ? `<button class="button primary" data-action="add-account" data-service="${esc(group.id)}" ${available ? '' : 'disabled'}>${icon('plus')} ${esc(group.name)}を接続</button>` : ''}</div>
    ${accounts.length ? `<div class="connection-workspace"><div class="account-list" role="group" aria-label="${esc(group.name)}の接続済みアカウント">${accounts.map(item => `<button class="account-item ${item.id === account.id ? 'selected' : ''}" data-action="select-account" data-id="${esc(item.id)}" aria-pressed="${item.id === account.id}"><strong>${esc(item.name)}</strong><span>${esc(accountLabel(item))}</span>${item.status !== 'connected' ? `<small class="warning-text">${statusName(item)}</small>` : ''}</button>`).join('')}</div><div class="connection-pane">${details(account)}</div></div>` : `<div class="empty-state"><span class="empty-icon">${icon(group.icon)}</span><div><h3>${esc(group.name)}を接続しましょう</h3><p>${esc(addable[0]?.intro || group.adapters[0].intro)}</p></div></div>`}
    ${addable.length && !available ? '<p class="availability" role="status">現在、新しい接続を追加できません。</p>' : ''}</section>`;
}
function render() {
  if (!state) return;
  clearPrivateInput();
  ...
  app.innerHTML = `<div class="workspace"><header class="topbar">${brand}<div class="user-menu"><span>${esc(state.user.email)}</span><button class="text-button" data-action="logout">ログアウト</button></div></header><main><header class="page-heading"><h1>接続</h1><p>アカウントとAIのアクセスキーを管理</p></header>
    ${serviceGroups().filter(group => group.adapters.some(adapter => adapter.available && !adapter.declared) || state.accounts.some(account => group.adapters.some(adapter => adapter.id === account.adapter))).map(serviceSection).join('')}
...
// The approval URL shows one of two screens. Registering an account is between the owner and the service, and
...
function renderRequest() {
  ...
  const finished = {
    approved: row?.account ? ['登録しました', `${row.account.label || row.account.subject} を、${row.requester_name}から利用できます。この画面は閉じて構いません。`] : ['承認しました', `${row?.requester_name || ''}から、登録済みの接続を利用できるようになりました。この画面は閉じて構いません。`],
  ...
  const adapter = row.adapter, name = serviceName(adapter, row.details);
  const same = state.accounts.filter(account => account.adapter === adapter.id && account.status !== 'disconnecting');
  ...
  if (screen === 'register') {
    const connectLabel = adapter.declared ? name + 'のキーを登録' : adapter.label;
    const unavailable = `<p class="form-error" role="status">現在${esc(name)}を接続できません。</p>`;
    const body = !adapter.available ? unavailable : adapter.register === 'login' ? expoLoginMarkup(row) : adapter.register === 'paste' ? schemaFormMarkup(row.form)
      : `${same.filter(account => account.status === 'reconnect_required' && adapter.can_reconnect).map(account => `<button class="button secondary full request-connect" type="button" data-action="request-connect" data-id="${esc(account.id)}">${esc(accountLabel(account))} を再接続</button>`).join('')}
        ...
        ${adapter.id === 'openrouter.oauth' && ['failed', 'scope', 'retry', 'changed'].includes(resultCode) ? '<p class="permission-note">接続できなくても、OpenRouterで作成済みのキーが残る場合があります。<a href="https://openrouter.ai/keys" target="_blank" rel="noopener noreferrer">不要なキーはOpenRouterで削除してください ↗</a></p>' : ''}`;
    ...
    if (adapter.available && adapter.register === 'paste') bindSchemaForm(container, row.form, async values => {
      let result;
      try { result = await api(`/api/adapters/${adapter.id}/connect`, { method: 'POST', data: { values, permission: row.permission.id, name, purpose: row.purpose, accessRequestId: row.id } }); }
  ...
  app.innerHTML = shell(`<section class="approval-card"><header class="approval-heading"><span class="approval-symbol">${icon('lock')}</span><div><p class="approval-eyebrow">${esc(name)}へのアクセス</p><h1>このアクセスキーを承認しますか？</h1></div></header>
    <dl class="approval-facts"><div><dt>依頼元</dt><dd>${esc(row.requester_name)}<span class="muted block">新しいアクセスキーです。承認すると、登録済みの接続をすべて使えるようになります。</span></dd></div>${row.purpose ? `<div><dt>用途</dt><dd>${esc(row.purpose)}</dd></div>` : ''}<div><dt>権限</dt><dd>${esc(row.permission.name)}${row.permission.restrictions ? `<span class="muted block">${esc(row.permission.restrictions)}</span>` : ''}</dd></div>${claimRows(adapter, row.details)}</dl>${adapter.declared ? '<p class="permission-note claim-note">サービス名と作成ページはAIの申告です。作成ページのドメインが正しいか確認してください。</p>' : ''}
    <form id="access-request-form">
    ${fresh ? `<h2 class="approval-section">この依頼で登録した接続</h2>${accountCard(fresh)}` : `<h2 class="approval-section">${esc(name)}の登録済みの接続</h2>${same.map(accountCard).join('')}`}
    <p class="permission-note">承認後に使える接続: ${usable.map(account => esc(serviceName(adapterOf(account), account.details)) + ' (' + esc(accountLabel(account) || account.name) + ')').join('、')}</p>
    <button class="text-button full" type="button" data-action="open-register" ${adapter.available ? '' : 'disabled'}>別のアカウントを登録する</button>
    ${codeField()}
...
// The dashboard's add button belongs to a service; with several adapters the owner picks how to connect first.
function chooseAdapter(serviceId) {
  const options = state.adapters.filter(adapter => (adapter.service?.id || 'generic') === serviceId && adapter.available && !adapter.declared);
  if (options.length === 1) { connect(null, options[0].id); return; }
  openDialog(`<h2 id="dialog-title">${esc(options[0].service.name)}を接続</h2><p>接続方法を選んでください。</p>${options.map(adapter => `<button class="button secondary full setup-link" data-action="add-adapter" data-adapter="${esc(adapter.id)}">${esc(adapter.label)}</button>`).join('')}`);
}
function connect(account, adapterId = account?.adapter) {
  const adapter = state.adapters.find(item => item.id === adapterId);
  if (!adapter?.available) return;
  const name = serviceName(adapter);
  if (adapter.register === 'login') { openDialog(`<h2 id="dialog-title">${esc(name)}を接続</h2>${expoLoginMarkup(null)}`); bindExpoLogin(dialog, null); return; }
  if (adapter.register === 'paste') {
    openDialog(`<h2 id="dialog-title">${esc(name)}を接続</h2><p>${esc(adapter.intro)}</p>${adapter.instructions ? `<p class="permission-note">${esc(adapter.instructions)}</p>` : ''}${adapter.note ? `<p class="permission-note">${esc(adapter.note)}</p>` : ''}${schemaFormMarkup(adapter.form)}`);
    bindSchemaForm(dialog, adapter.form, async values => {
      const result = await api(`/api/adapters/${adapter.id}/connect`, { method: 'POST', data: { values, permission: adapter.permissions[0].id, name, purpose: '' } });
      selected = result.account_id; closeDialog(); await refresh(); toast(`${name}の認証情報を登録しました。`);
    });
    return;
  }
  const chosen = account?.permission?.id || adapter.permissions[0].id;
  openDialog(`<h2 id="dialog-title">${esc(name)}を${account ? '再接続' : '接続'}</h2><p>${account ? esc(accountLabel(account)) : esc(adapter.intro)}</p><form>${accountFields(account || { name })}
    <fieldset><legend>利用する権限</legend>${adapter.permissions.map(permission => `<label class="choice"><input type="radio" name="permission" value="${esc(permission.id)}" ${chosen === permission.id ? 'checked' : ''}><span><strong>${esc(permission.name)}</strong><small>${esc(permission.description)}</small></span></label>`).join('')}</fieldset>
    <p class="permission-note"><span id="permission-restrictions">${esc(adapter.permissions.find(permission => permission.id === chosen)?.restrictions)}</span> ${account ? '' : '接続すると、承認済みのアクセスキーから使えるようになります。'}${adapter.can_revoke ? '' : `キーの停止は${esc(name)}で行います。`}</p><p class="form-error" role="alert"></p><button class="button primary full" type="submit">${esc(adapter.label)} ${icon('arrow')}</button></form>`);
  dialog.querySelector('form').addEventListener('change', event => {
    if (event.target.name === 'permission') dialog.querySelector('#permission-restrictions').textContent = adapter.permissions.find(permission => permission.id === event.target.value)?.restrictions || '';
  });
  bindForm(async (form) => {
    const result = await api(`/api/adapters/${adapter.id}/connect`, { method: 'POST', data: { name: form.get('name'), purpose: form.get('purpose'), permission: form.get('permission'), ...(account ? { accountId: account.id } : {}) } });
...
function expoLoginMarkup(request) {
  return `<form class="expo-login-form" autocomplete="off"><div class="expo-password-fields"><label for="expo-username">Expoのメールアドレスまたはユーザー名</label><input id="expo-username" name="username" required maxlength="254" autocomplete="off" autocapitalize="none" spellcheck="false"><label for="expo-password">パスワード</label><input id="expo-password" name="password" type="password" required maxlength="1024" autocomplete="off"></div>
    ...
    <button class="button primary full" type="submit" ${request && !request.adapter.available ? 'disabled' : ''}>${request && !request.agent_name ? 'ログインして承認' : 'ログインして接続'} ${icon('arrow')}</button></form>`;
}
function bindExpoLogin(container, request) {
  ...
  function reset() {
    ...
    button.disabled = Boolean(request && !request.adapter.available); button.textContent = request && !request.agent_name ? 'ログインして承認' : 'ログインして接続';
  }
  ...
  form.addEventListener('submit', async event => {
    ...
    try {
      const result = await api('/api/adapters/expo.login/connect', { method: 'POST', signal, data: { name: 'Expo', purpose: request?.purpose || '', permission: 'session', username, password, ...(otp === undefined ? {} : { otp }), ...(request ? { accessRequestId: request.id, ...(form.elements.confirmationCode ? { confirmationCode: form.elements.confirmationCode.value } : {}) } : {}) } });
...
function removeAccount(account) {
  ...
  const revoke = expoSession ? '<p>この接続のExpoログインを無効にします。承認済みのアクセスキーからは使えなくなります。Expoのプロジェクトやデータは削除しません。</p>' : adapter.can_revoke ? `<p>承認済みのアクセスキーからは使えなくなります。メールは削除されません。</p><label class="choice revoke-choice"><input type="checkbox" name="revoke" checked><span><strong>Google側の許可も取り消す</strong><small>このアプリに与えた、ほかのGoogleサービスの許可も取り消されます。反映に時間がかかる場合があります。</small></span></label><p class="permission-note">チェックを外すと、Googleの許可は残ります。${revocationNote}</p>` : `<p>Foundationから、この接続を削除します。承認済みのアクセスキーからは使えなくなります。受け渡し済みのAPIキーは、この操作では無効になりません。</p><p><a href="${esc(account.management_url || adapter.service?.management_url)}" target="_blank" rel="noopener noreferrer">${esc(name)}でキーを削除する ↗</a></p><label class="choice"><input type="checkbox" name="acknowledged" required><span>キーの無効化は${esc(name)}で行うことを確認しました</span></label>`;
  openDialog(`<h2 id="dialog-title">${esc(name)}の接続を解除しますか？</h2><p>${esc(accountLabel(account))}</p><form>${revoke}<p class="form-error" role="alert"></p><div class="dialog-actions"><button type="button" class="button secondary" data-action="close-dialog">キャンセル</button><button type="submit" class="button destructive">接続を解除</button></div></form>`);
  bindForm(async (form) => {
...
document.addEventListener('click', async (event) => {
  ...
  try {
    ...
    if (action === 'request-connect') {
      ...
      const result = await api(`/api/adapters/${accessRequest.adapter.id}/connect`, { method: 'POST', data: { name: account?.name || serviceName(accessRequest.adapter, accessRequest.details), purpose: account?.purpose || accessRequest.purpose, permission: accessRequest.permission.id, accessRequestId: requestId, ...(account ? { accountId: account.id } : {}) } });
      location.assign(result.url);
    }
    ...
    if (action === 'add-account') chooseAdapter(target.dataset.service);
    ...
    if (action === 'check') {
      ...
      try {
        ...
        toast(result.verification?.checks.some(item => item.status !== 'passed' && item.check !== 'permissions') ? '検証結果を更新しました。確認できなかった項目があります。' : serviceName(adapterOf(state.accounts.find(account => account.id === id)), state.accounts.find(account => account.id === id).details) + 'に接続できました。');
      }
...
} else if (resultAdapter !== 'gmail.oauth' && resultCode) {
