...
function renderRequest() {
  ...
  app.innerHTML = shell(`<section class="approval-card"><header class="approval-heading"><span class="approval-symbol">${icon('lock')}</span><div><p class="approval-eyebrow">${esc(row.service.name)}へのアクセス</p><h1>利用を許可しますか？</h1></div></header>
    <dl class="approval-facts"><div><dt>依頼元</dt><dd>${esc(row.requester_name)}<span class="muted block">${row.agent_name ? row.agent_name === row.requester_name ? '承認済みのアクセスキーからの依頼です。' : `承認済みのアクセスキー「${esc(row.agent_name)}」からの依頼です。キーの名前は変わりません。` : '新しいアクセスキーです。許可すると登録されます。'}</span></dd></div>${row.purpose ? `<div><dt>用途</dt><dd>${esc(row.purpose)}</dd></div>` : ''}<div><dt>権限</dt><dd>${esc(row.permission.name)}${row.permission.restrictions ? `<span class="muted block">${esc(row.permission.restrictions)}</span>` : ''}</dd></div>${claimRows(row.service, row.details)}</dl>${row.service.request_fields?.length ? '<p class="permission-note claim-note">サービス名・作成ページ・環境変数名はAIの申告です。作成ページのドメインが正しいか確認してください。</p>' : ''}
    <form id="access-request-form"><fieldset><legend>利用を許可するアカウント</legend>${available.length ? available.map((account) => `<div class="approval-account"><label class="choice"><input type="radio" name="accountId" value="${esc(account.id)}" ${available.length === 1 ? 'checked' : ''} required><span><strong>${esc(account.name)}</strong><small>${esc(accountLabel(account))}</small></span></label>${keyFacts(account)}${verificationDetails(account.verification)}</div>`).join('') : '<p class="muted">この権限で利用できる接続済みのアカウントはありません。</p>'}</fieldset>
  ...
  form.addEventListener('submit', async (event) => {
    ...
    try {
      await api(`/api/access-requests/${requestId}/approve`, { method: 'POST', data: { accountId: new FormData(form).get('accountId'), confirmationCode: form.elements.confirmationCode.value } });
      await refresh();
...
function connect(account, providerId = account?.provider) {
  ...
  openDialog(`<h2 id="dialog-title">${esc(provider.name)}を${account ? '再接続' : '接続'}</h2><p>${account ? esc(accountLabel(account)) : esc(provider.intro)}</p><form>${accountFields(account || { name: provider.name })}
    ...
    <p class="permission-note"><span id="permission-restrictions">${esc(provider.permissions.find(permission => permission.id === mode)?.restrictions)}</span> ${account ? '許可範囲を変更した場合は、アクセスキーへの利用許可を設定し直してください。' : '接続するだけでは、アクセスキーに利用を許可しません。'}${provider.can_revoke ? '' : `キーの停止は${esc(provider.name)}で行います。`}</p><p class="form-error" role="alert"></p><button class="button primary full" type="submit">${esc(provider.connect_label)} ${icon('arrow')}</button></form>`);
...
function expoLoginMarkup(request) {
  return `<form class="expo-login-form" autocomplete="off"><div class="expo-password-fields"><label for="expo-username">Expoのメールアドレスまたはユーザー名</label><input id="expo-username" name="username" required maxlength="254" autocomplete="off" autocapitalize="none" spellcheck="false"><label for="expo-password">パスワード</label><input id="expo-password" name="password" type="password" required maxlength="1024" autocomplete="off"></div>
    ...
    ${request ? codeField() : ''}<p class="permission-note auth-privacy">入力内容はFoundationを経由してExpoへ送信します。パスワード・認証コードは保存しません。</p>
...
function bindExpoLogin(container, request) {
  ...
  form.addEventListener('submit', async event => {
    ...
    try {
      const result = await api('/api/connections/expo/login', { method: 'POST', signal, data: { username, password, ...(otp === undefined ? {} : { otp }), ...(request ? { accessRequestId: request.id, confirmationCode: form.elements.confirmationCode.value } : {}) } });
...
function renderExpoRequest(row, shell) {
  ...
  area.querySelector('form').addEventListener('submit', async event => {
    ...
    try { await api(`/api/access-requests/${row.id}/approve`, { method: 'POST', data: { accountId: new FormData(form).get('accountId'), confirmationCode: form.elements.confirmationCode.value } }); await refresh(); }
...
function connectToken(provider, request = null) {
  ...
  openDialog(`<h2 id="dialog-title">${esc(serviceName || provider.name)}を接続</h2><p>${setupFields.length ? `${esc(provider.name)}で発行した${esc(setup.label)}と、対象の${setupFields.map(field => esc(field.label)).join('・')}を登録します。` : serviceName ? `パスワードは${esc(serviceName)}の画面で入力します。ここには${esc(setup.label)}だけを登録してください。` : esc(provider.intro)}</p>
    ...
    <div class="token-setup"><h3>1. ${esc(setup.step_label || `${serviceName || 'サービス'}でキーを作成`)}</h3><p>${esc(setup.instructions)}</p>${siteUrl || fields.length ? `<a class="button secondary full" href="${esc(siteUrl || '#')}" target="_blank" rel="noopener noreferrer" ${siteUrl ? '' : 'style="display:none"'}>${esc(setup.link_label || `${serviceName || 'サービス'}の${setup.label}管理ページを開く`)} ↗</a>` : ''}</div>
  ...
  dialog.querySelector('#connection-token-file')?.addEventListener('change', async event => {
    const file = event.target.files?.[0]; if (!file) return;
    const text = file.size <= 8192 ? await file.text() : '';
    dialog.querySelector('[name="token"]').value = text.trim();
    dialog.querySelector('.form-error').textContent = text ? '' : 'ファイルが大きすぎます。.p8 ファイルを選んでください。';
    event.target.value = '';
  });
