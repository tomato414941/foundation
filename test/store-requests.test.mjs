...
test('利用者が選んだ名前で保存し、依頼元に実際の保存名を返す', async t => {
  ...
  const kept = f.app.grants.list(USER_A);
...
test('同じ名前を使う登録を全件保留し、既存の値を保ったまま別名で再試行する', async t => {
  const f = await fixture(t), { token } = await f.issueKey();
  await f.request('/v1/holdings?kind=grant&name=existing', { method: 'PUT', raw: 'keep-this-value', type: 'text/plain' });
  const before = f.app.grants.list(USER_A);
  ...
  assert.deepEqual(f.app.grants.list(USER_A), before);
  assert.equal(f.app.grants.content(f.app.grants.find(USER_A, 'existing')).toString(), 'keep-this-value');
  ...
  assert.equal((await f.read('grant', 'other-name', { token })).text, 'replacement');
...
test('依頼された名前をそのまま使う場合も同名の値を保護する', async t => {
  ...
  await f.request('/v1/holdings?kind=grant&name=existing', { method: 'PUT', raw: 'original', type: 'text/plain' });
  const refused = await save(f, row, [entry('existing')]);
  assert.equal(refused.status, 409, refused.text);
  assert.equal(f.app.grants.content(f.app.grants.find(USER_A, 'existing')).toString(), 'original');
});
...
test('同じ保存名への同時登録は一方だけを保存し、もう一方を保留する', async t => {
  ...
  assert.equal(f.app.grants.content(f.app.grants.find(USER_A, 'shared')).toString(), ['first-value', 'second-value'][winner]);
...
test('置き換えの依頼は、持ち主がそのままの名前で完了すると既存の値だけを入れ替え、線はそのまま保つ', async t => {
  const f = await fixture(t), { token } = await f.issueKey();
  await f.request('/v1/holdings?kind=grant&name=npm-token', { method: 'PUT', raw: 'old-value', type: 'text/plain' });
  const before = f.app.grants.find(USER_A, 'npm-token');
  ...
  const after = f.app.grants.find(USER_A, 'npm-token');
  assert.equal(f.app.grants.content(after).toString(), 'new-value');
  assert.equal((await f.read('grant', 'npm-token', { token })).status, 403, 'the request cannot loosen what the owner kept to themselves');
  assert.equal(after.id, before.id, 'the same value, updated');
  assert.equal(f.app.grants.list(USER_A).length, 1);
});
...
test('置き換えの依頼でも持ち主が別の名前を付ければ、既存の値は残り新しく保管される', async t => {
  ...
  assert.equal(f.app.grants.content(f.app.grants.find(USER_A, 'npm-token')).toString(), 'old-value');
  assert.equal(f.app.grants.content(f.app.grants.find(USER_A, 'npm-token-2')).toString(), 'new-value');
  const done = (await f.request(`/v1/requests/${row.id}`, { token })).json.request;
