...
test('答えは subject・action・resource から decision だけを返し、根拠は関係にある', () => {
  ...
  assert.equal(ask(me, 'content', { type: 'grant', id: 'x', holder: person.id }), true, 'the holder');
  assert.equal(ask(actor, 'list', { type: 'grant', holder: person.id }), true, 'one who acts for the holder reaches their things');
  assert.equal(ask(actor, 'read', { type: 'grant', id: 'x', holder: person.id }), true, 'and sees what each is');
  assert.equal(ask(actor, 'content', { type: 'grant', id: 'x', holder: person.id }), false, 'but reads what one holds only along a line to it');
  assert.equal(ask(stranger, 'content', { type: 'grant', id: 'x', holder: person.id }), false, 'nobody else');
  assert.equal(ask(actor, 'rename', { type: 'grant', id: 'x', holder: person.id }), false, 'renaming is the holder\'s alone');
  principals.relate(other.id, 'viewer', 'holding', 'x');
  assert.equal(ask(stranger, 'content', { type: 'grant', id: 'x', holder: person.id }), true, 'a line drawn onto the thing itself');
  assert.equal(ask(stranger, 'content', { type: 'grant', id: 'y', holder: person.id }), false, 'and only that thing');
  assert.equal(ask(stranger, 'write', { type: 'grant', id: 'x', holder: person.id }), false);
  ...
  assert.equal(ask(actor, 'connect', { type: 'grant', holder: person.id }), false, 'a browser is needed for a service\'s consent screen');
  assert.equal(ask(me, 'connect', { type: 'grant', holder: person.id }), true);
  ...
  assert.equal(ask(linked, 'list', { type: 'grant', holder: person.id }), false, 'and nothing else');
  assert.deepEqual(authorization.allowed({}), { decision: false });
  assert.ok(rules().some(rule => rule.resource === 'grant' && rule.action === 'rename' && rule.grounds.join() === 'self'));
  store.close();
});
...
test('ルートは同じ問いを立て、許されない主体には 403、依頼だけを渡された利用者には 401 で答える', async t => {
  ...
  assert.equal((await f.request('/v1/holdings?kind=grant', { token: key.token, anonymous: true })).status, 200, 'what both may do still works');
  assert.equal((await f.request('/v1/holdings?kind=grant')).status, 200);
