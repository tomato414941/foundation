...
import { AppleProvider, APPLE_API } from '../src/providers/apple.mjs';
import { appleConnection, gmailConnection } from '../src/providers/catalog.mjs';
...
export class FakeApple extends AppleProvider {
  constructor() {
...
export async function appleFixture(t, options = {}) {
  ...
  const f = await fixture(t, { gmail, integrations: [appleConnection(apple), gmailConnection(gmail)], ...options });
  const importApple = (extra = {}) => f.request('/api/connections/apple/connect', { method: 'POST', data: { name: 'Apple', mode: 'api-key', token: APPLE_P8, fields: APPLE_FIELDS, ...extra } });
