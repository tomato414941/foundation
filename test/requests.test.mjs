...
const approve = (f, row, overrides = {}) => f.request('/v1/key-requests/' + row.id + '/approve', { method: 'POST', data: { confirmation_code: row.confirmation_code, ...overrides } });
...
test('A new key asks only to be approved: no access before approval, the same private key after it', async t => {
  ...
  assert.equal(row.verification_uri, f.base + '/key-requests/' + row.id);
  assert.match(row.confirmation_code, /^[A-F0-9]{4}-[A-F0-9]{4}$/);
  assert.doesNotMatch(JSON.stringify(row), /fdn_|token_hash|refresh_token/);
  assert.equal((await f.request('/key-requests/' + row.id, { anonymous: true, headers: { 'sec-fetch-site': 'cross-site' } })).status, 200);
  assert.equal((await f.request('/v1/key-requests/' + row.id, { anonymous: true })).status, 401);
...
test('Email login returns to the exact request page and rejects open redirects', async t => {
  ...
  for (const return_to of ['https://evil.test/', '//evil.test/', '/keys/x', '/key-requests/' + row.id + '?next=evil', '/key-requests/' + row.id + '/..', 42]) {
  ...
  const sent = await f.request('/v1/login', { method: 'POST', data: { email: 'owner@example.test', return_to: '/key-requests/' + row.id } });
  ...
  assert.equal(callback.headers.get('location'), '/key-requests/' + row.id);
});
...
test('An approval request belongs to the owner who approves it', async t => {
  ...
  await f.login('other@example.test');
  assert.equal((await f.request('/v1/key-requests/' + row.id)).status, 404);
  assert.equal((await f.request('/v1/key-requests/' + row.id + '/deny', { method: 'POST', data: {} })).status, 404);
});
...
test('Cross-site creation, invalid names and cross-origin approval are rejected', async t => {
  ...
  const response = await f.request('/v1/key-requests/' + row.id + '/approve', { method: 'POST', headers: { origin: 'https://evil.test' }, data: { confirmation_code: row.confirmation_code } });
...
test('The approval page never receives the confirmation code; entry is normalized and locked after repeated mistakes', async t => {
  const f = await fixture(t);
  const { row } = await create(f);
  const page = await f.request('/v1/key-requests/' + row.id);
...
test('An access key introduces itself: whoami, the owner can rename it, it can rename itself, and it can leave', async t => {
  ...
  const before = await f.request('/v1/keys/current', { token, anonymous: true });
  assert.equal(before.status, 200); assert.equal(before.json.request.status, 'pending'); assert.equal(before.json.key, undefined, 'nothing to be yet');
  assert.equal((await approve(f, row)).status, 200);
  const me = await f.request('/v1/keys/current', { token, anonymous: true });
...
test('A runtime can read its own request raw: what it asked for, and what happened at its page, never an input', async t => {
  ...
  assert.equal((await f.request('/key-requests/' + row.id, { anonymous: true })).status, 200);
  await f.login();
  await f.request('/v1/key-requests/' + row.id);
