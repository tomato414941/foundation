...
test('hands the owner everything they have, secrets included', async (t) => {
  ...
  await f.request('/v1/holdings?kind=grant&name=notes/plan', { method: 'PUT', token: KEY, raw: 'read me', type: 'text/plain' });
  await f.request('/v1/holdings?kind=grant&name=keys/token', { method: 'PUT', token: KEY, raw: 'sh-secret-value', type: 'text/plain' });
  ...
  const byPath = Object.fromEntries(value.grants.map(entry => [entry.name, entry]));
...
test('keeps the export to the owner\'s own session', async (t) => {
  ...
  assert.deepEqual(asAnother.json.grants, []);
});
