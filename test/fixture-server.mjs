...
import { gmailOauth, openrouterOauth, expoToken, expoLogin, generic, supabaseAccessToken, cloudflareApiToken, appleApiKey, awsIamUserKey } from '../src/adapters.mjs';
...
const adapters = process.env.FOUNDATION_TEST_CLOUDFLARE === '1' ? [cloudflareApiToken(new FakeCloudflare()), gmailOauth(gmail)] : process.env.FOUNDATION_TEST_EXPO_LOGIN === '1' ? [expoLogin(new FakeExpoLogin()), gmailOauth(gmail)] : process.env.FOUNDATION_TEST_EXPO === '1' ? [expoToken(new FakeExpo()), gmailOauth(gmail)] : process.env.FOUNDATION_TEST_APIKEY === '1' ? [generic(new GenericClient()), gmailOauth(gmail)] : process.env.FOUNDATION_TEST_AWS === '1' ? [awsIamUserKey(new FakeAws()), gmailOauth(gmail)] : process.env.FOUNDATION_TEST_APPLE === '1' ? [appleApiKey(new FakeApple()), gmailOauth(gmail)] : process.env.FOUNDATION_TEST_SUPABASE === '1' ? [supabaseAccessToken(new FakeSupabase()), gmailOauth(gmail)] : process.env.FOUNDATION_TEST_OPENROUTER === '1' ? [openrouterOauth(new FakeOpenRouter()), gmailOauth(gmail)] : undefined;
const app = createApp({ encryptionKey: KEY, auth, adapters: adapters || [gmailOauth(gmail)] });
