...
test('lets the owner change what something is called, without the value being handed back', async (t) => {
  ...
  await f.request('/v1/secrets?name=cloudflare/registrar-api-token&secret=true',
...
test('refuses to move something onto a name already in use', async (t) => {
  ...
  for (const path of ['a/one', 'a/two']) {
    await f.request('/v1/secrets?name=' + path + '&secret=true', { method: 'PUT', token: KEY, raw: 'x', type: 'text/plain' });
  }
...
test('hands the same value over under whatever names the caller asks for', async (t) => {
  const f = await fixture(t);
  await f.approveKey(KEY);
  await f.request('/v1/secrets?name=github/token&secret=true', { method: 'PUT', token: KEY, raw: 'ghp_value', type: 'text/plain' });
...
test('requires the caller to specify the delivery variable', async (t) => {
  const f = await fixture(t);
  await f.approveKey(KEY);
  await f.request('/v1/secrets?name=notes/2026-09-23&secret=true', { method: 'PUT', token: KEY, raw: 'x', type: 'text/plain' });
...
test('lets the owner put something there themselves', async (t) => {
  ...
  const open = await f.request('/v1/secrets?name=' + encodeURIComponent('aws/region') + '&secret=false',
