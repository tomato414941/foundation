...
for (const scope of ['https://mail.google.com/', 'https://www.googleapis.com/auth/gmail.modify', 'https://www.googleapis.com/auth/gmail.send', 'https://www.googleapis.com/auth/drive']) test('Broad or unrelated Google scope rejected: ' + scope, async (t) => {
  ...
  await assert.rejects(run(), (error) => error.code === 'scope_mismatch' && !error.message.includes('secret-broad'));
  assert.equal(account().status, 'reconnect_required');
  assert.notEqual(state().renewal.access_token, 'secret-broad-access');
});
...
test('Metadata-only grant cannot be silently expanded to body access', async (t) => {
  ...
  await assert.rejects(run(), { code: 'scope_mismatch' });
});

test('Missing scope on initial exchange and missing refresh token are rejected', () => {
  const gmail = new FakeGmail();
  assert.throws(() => gmail.grant({ access_token: 'a', refresh_token: 'r', expires_in: 3600 }, 'readonly'), { code: 'scope_mismatch' });
  assert.throws(() => gmail.grant({ access_token: 'a', scope: READONLY_SCOPE, expires_in: 3600 }, 'readonly'), { code: 'refresh_missing' });
});
...
test('Refresh may omit unchanged scopes and refresh token under OAuth specification', async (t) => {
  ...
  await run();
  assert.equal(state().renewal.refresh_token, 'refresh-personal-readonly');
  assert.deepEqual(state().renewal.scopes, [READONLY_SCOPE]);
});
...
test('Malformed token lifetimes, bearer types and identity are rejected', () => {
  const gmail = new FakeGmail();
  for (const change of [{ expires_in: -1 }, { expires_in: 900000 }, { expires_in: '3600' }, { access_token: '' }, { access_token: 'a\r\nb' }, { token_type: 'unknown' }]) assert.throws(() => gmail.grant({ access_token: 'a', refresh_token: 'r', scope: READONLY_SCOPE, expires_in: 3600, ...change }, 'readonly'), { code: 'service_response' });
});
