...
with sync_playwright() as p:
    ...
    dialog.get_by_role('button', name='製品キーを発行').click()
    expect(dialog.get_by_role('heading', name='ai-simplicity の製品キー', exact=True)).to_be_visible()
    product = dialog.get_by_label('製品キー', exact=True).input_value()
