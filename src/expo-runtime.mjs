...
export async function spawnExpoSession(command, environment, credential) {
  ...
  const profile = credential.session_profile;
  ...
  const auth = { sessionSecret: credential.access_token, userId: profile.user_id, username: profile.username, currentConnection: 'Username-Password-Authentication' };
