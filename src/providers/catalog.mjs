...
export function supabaseConnection(client) {
  return {
    ...
    tokenSetup: { links: [{ label: 'Supabaseのアクセストークン管理ページを開く', href: SUPABASE_TOKENS }], label: 'アクセストークン', instructions: 'Supabaseのアカウントのアクセストークン (sbp_ で始まる) を受け付けます。登録時にSupabaseへ照会して持ち主を確認します。' },
...
export function apikeyConnection(client) {
  return {
    ...
    tokenSetup: { label: 'APIキー', instructions: '任意のサービスのキーを 1 行で受け付けます。Foundationは検証しません。' },
