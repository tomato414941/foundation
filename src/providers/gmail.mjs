...
export class GmailProvider {
  ...
  scope(mode) {
    if (mode === 'readonly') return READONLY_SCOPE;
    if (mode === 'metadata') return METADATA_SCOPE;
    fail(400, 'invalid_scope', '読み取り範囲を選んでください。');
  }
  authorize({ state, verifier, redirectUri, mode, email }) {
    this.check();
    const url = new URL('https://accounts.google.com/o/oauth2/v2/auth');
    url.search = new URLSearchParams({
      client_id: this.clientId, redirect_uri: redirectUri, response_type: 'code',
      scope: this.scope(mode), access_type: 'offline', prompt: 'consent select_account',
  ...
  async request(url, options = {}) {
    ...
    catch { fail(502, 'provider_unavailable', 'Googleに接続できませんでした。時間をおいて再度お試しください。'); }
    let data;
    try { data = await response.json(); } catch { data = null; }
    if (!response.ok) {
      if (data?.error === 'invalid_grant' || response.status === 401) fail(409, 'reconnect_required', 'Gmailへの再接続が必要です。');
      if (response.status === 429) fail(503, 'provider_rate_limit', 'Googleの利用上限に達しました。時間をおいて再度お試しください。');
      fail(502, 'provider_unavailable', 'Googleで処理を完了できませんでした。');
    }
    if (!data || typeof data !== 'object') fail(502, 'provider_response', 'Googleからの応答を確認できませんでした。');
    return data;
  }
  ...
  credentials(data, mode, previous) {
    const requested = this.scope(mode);
    const scopes = typeof data.scope === 'string' ? [...new Set(data.scope.trim().split(/\s+/))].sort() : previous?.scopes;
    const allowed = new Set([...IDENTITY_SCOPES, METADATA_SCOPE, ...(mode === 'readonly' ? [READONLY_SCOPE] : [])]);
    ...
    if (typeof data.access_token !== 'string' || !data.access_token || data.access_token.length > 8192 || /[\r\n]/.test(data.access_token) || !Number.isFinite(data.expires_in) || data.expires_in <= 0 || data.expires_in > 86400 || (data.token_type && (typeof data.token_type !== 'string' || data.token_type.toLowerCase() !== 'bearer'))) fail(502, 'provider_response', 'Googleの認証応答を確認できませんでした。');
  ...
  async identity(accessToken) {
    ...
    if (typeof data.emailAddress !== 'string' || data.emailAddress.length > 254 || !/^[^\s@]+@[^\s@]+$/.test(data.emailAddress)) fail(502, 'provider_response', 'Gmailのアドレスを確認できませんでした。');
    return data.emailAddress.toLowerCase();
  }
  async exchange({ code, verifier, redirectUri, mode }, previous) {
    const data = await this.tokenRequest({ code, code_verifier: verifier, redirect_uri: redirectUri, grant_type: 'authorization_code' });
    if (typeof data.access_token !== 'string' || !data.access_token || data.access_token.length > 8192 || /[\r\n]/.test(data.access_token)) fail(502, 'provider_response', 'Googleの認証応答を確認できませんでした。');
    // Verify identity before allowing reuse of an existing refresh token.
    const email = await this.identity(data.access_token);
    if (previous && previous.email !== email) fail(409, 'account_changed', '再接続には同じGoogleアカウントを選んでください。');
    return { email, credentials: this.credentials(data, mode, previous?.credentials) };
  }
  async token(store, account, force = false) {
    ...
    const pending = (async () => {
      try {
        ...
        if (await this.identity(next.access_token) !== account.email) fail(409, 'account_changed', 'Gmailのアカウントが変わりました。接続を確認してください。');
