...
const app = createApp({ database: config.database, encryptionKey, files: new S3Files(config.files), space: new S3Space(config.objects), publicOrigin: config.publicOrigin, owners: config.owners, trustedProxies: config.trustedProxies, auth, adapters });
app.server.listen(config.port, config.bind, () => {
  ...
  console.log(`Email login: ${auth.emailEnabled ? 'enabled' : 'disabled'}; Shared files: ${config.files.bucket || 'not configured'}; Object space: ${config.objects.bucket || 'not configured'}`);
