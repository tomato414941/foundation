...
import { gmailConnection, openrouterConnection, expoConnection, apikeyConnection, supabaseConnection, cloudflareConnection, appleConnection } from './providers/catalog.mjs';
...
const app = createApp({ database: config.database, encryptionKey: config.encryptionKey, publicOrigin: config.publicOrigin, auth, gmail, integrations: [openrouterConnection(openrouter), expoConnection(new ExpoProvider(config.expo)), supabaseConnection(new SupabaseProvider()), cloudflareConnection(new CloudflareProvider()), appleConnection(new AppleProvider()), gmailConnection(gmail), apikeyConnection(new ApiKeyProvider())] });
