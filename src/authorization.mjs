...
// scoped to one request reaches that request and nothing else. Lines onto a held thing point at its id.
...
const LINE = relation => (subject, resource, principals) => Boolean(resource.id !== undefined && principals.has(subject.id, relation, 'holding', resource.id));
...
const RULES = {
  ...
  secret: { list: [SELF, ACTOR], read: [SELF, ACTOR, LINE('viewer'), LINE('editor')], write: [SELF, ACTOR, LINE('editor')], remove: [SELF, ACTOR], rename: [SELF], share: [SELF] },
