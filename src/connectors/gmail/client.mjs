...
export class GmailClient extends GoogleOAuth {
  ...
  authorize({ range, email, ...context }) { return super.authorize({ ...context, scopes: [this.scope(range)], loginHint: email }); }
  ...
  facts(secret, subject, range) {
    ...
    const expected = new Set([...IDENTITY_SCOPES, METADATA_SCOPE, ...(range === 'readonly' ? [READONLY_SCOPE] : [])]);
    return { label: subject, scopes: secret.scopes,
      missing_scopes: secret.scopes.includes(requested) ? [] : [requested],
