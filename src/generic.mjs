...
// cannot tell what they can do, so nothing here claims an identity or a permission.
export class GenericClient {
  constructor() { this.enabled = true; }
  check() {}
  // `reserved` holds the variables other adapters deliver; a declared field may not take one.
  details(input, reserved = new Set()) {
    ...
    const fields = input.fields.map(field => {
      if (!field || typeof field !== 'object' || !validEnvName(field.id) || reserved.has(field.id)) fail(400, 'invalid_env', '環境変数名は英大文字・数字・下線で指定してください。予約された名前は使えません。');
  ...
  variables(details) { return details.fields.map(field => field.id); }
  async importToken({ values, details }) {
  ...
  async token(store, credential) {
    if (credential.status !== 'connected') fail(409, 'reconnect_required', '新しいキーで登録し直してください。');
    return store.secret(credential);
  }
  facts(secret) {
