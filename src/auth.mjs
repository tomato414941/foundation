...
export class SupabaseAuth {
  ...
  async sendLink(email, redirectUri, storage) {
    ...
    if (!(storage instanceof Map)) throw new Error('An isolated login store is required');
    const { error } = await this.client(storage).auth.signInWithOtp({ email, options: { shouldCreateUser: true, emailRedirectTo: redirectUri } });
  ...
  async exchangeLink(code, storage) {
    this.check();
    if (!(storage instanceof Map)) throw new Error('An isolated login store is required');
    const { data, error } = await this.client(new Map(storage)).auth.exchangeCodeForSession(code);
    if (error?.status === 429) fail(429, 'login_rate_limit', 'しばらく待ってからお試しください。');
