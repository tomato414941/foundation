...
export function guide(adapters) {
  return ['Foundation keeps what belongs to the owner, under the owner\'s control. Two things live here, and either one works without the other.',
    ...
    '  Storage: keep values you obtained yourself, and notes you need to survive this conversation (see STORAGE). An approved key may use it at any time, with no request.',
    ...
    '  Values: what a command reads as environment variables. Where a key or a setting you obtained yourself survives the conversation.',
    '   foundation keep --service <service> --value <NAME>=<value> [--value ...]   Keep them. NAME is exactly the environment variable the command will read.',
    '   `foundation values` lists them (ids and names only; values are never returned). `foundation forget <id>` removes one.',
    '   foundation exec <id> -- <command>   Delivers them. Ids of kept values and of credentials may be mixed in one exec.',
    '   Foundation does not check them. It cannot tell you a value has stopped working, so when a command fails on one, replace it yourself.',
    '  Documents: what you read back whole. JSON in, the same JSON out.',
    '   foundation write <collection>/<name> [--file <path>]   Writes JSON from stdin or --file (same name replaces it, up to 256KB).',
    '   `foundation read <collection>/<name>` reads one, `foundation documents [--collection <collection>]` lists them, `foundation erase <collection>/<name>` removes one.',
    '   Keep what the next conversation needs: where a procedure got to, what was decided, what is next. Keys and tokens belong in values, not here.',
    '  The owner sees both on their dashboard, and can read and delete anything in them at any time.', '',
    'TOOL: file space (use it or not; nothing in Foundation depends on it)',
    '   foundation put <file> [--type <content-type>] [--minutes <n>]   Puts a file and returns a time-limited read URL. Up to 5MB, gone after 7 days.',
    '   Anyone with the URL can read that one file (no writing, no listing). A file you put cannot be changed. Put another one instead.',
    '   The URL is an S3 presigned URL, so it works anywhere an S3 URL is required (CloudFormation quickcreate?templateURL=, for one).',
    '   `foundation files` lists them, `foundation link <file-id> [--minutes <n>]` issues the URL again (default 60 minutes, max 10080). Never put a secret there.', '',
    'RULES',
    ...
    'Other commands: foundation adapters | credentials | values | documents | files | request | cancel | whoami | rename <name> | leave (revokes your own key)',
