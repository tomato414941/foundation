...
export function guide(adapters) {
  return ['Foundation keeps what belongs to the owner, under the owner\'s control. Two things live here, and either one works without the other.',
    ...
    '2. If `foundation credentials` already has what you need, skip to 4. Otherwise ask for one.',
    '   Foundation performs only the acquisitions nobody else can: an OAuth exchange that needs the operator\'s own client secret, or a login relayed once.',
    '   Those are the adapters below. For anything the owner can simply go and fetch themselves -- an API token, a key, a certificate -- there is no adapter:',
    '   ask them for it instead (see ASKING), and Foundation holds no knowledge of that service at all.',
    ...
    'ASKING (when the owner has to go and get it: no adapter, and nothing about the service is known here)',
    ...
    '     <path>      where it will be kept, in the same space as everything else you keep',
    '     --label     what the owner is being asked for, in their language. It titles the screen and names the field.',
    ...
    '     --guide     the steps they follow, which you look up now and write yourself',
    ...
    '   Ask for what the owner already holds. Never take it through the conversation and keep it yourself.', '',
    ...
    '  Keep what you obtained yourself. For what the owner already holds, use ask, so it never passes through you.',
